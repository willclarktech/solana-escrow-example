...
impl Processor {
	...
	fn process_exchange(
		...
		let taker_temp_token_account = next_account_info(account_info_iter)?;

		let taker_token_to_receive_account = next_account_info(account_info_iter)?;
		if *taker_token_to_receive_account.owner != spl_token::id() {
			return Err(ProgramError::IncorrectProgramId);
		}
		...
		if pda_temp_token_account_info.amount != expected_amount {
		...
		if *pda_temp_token_account.key != escrow_info.temp_token_account_pubkey {
			return Err(ProgramError::InvalidAccountData);
		}
		if *initializer.key != escrow_info.initializer_pubkey {
			return Err(ProgramError::InvalidAccountData);
		}
		if *initializer_token_to_receive_account.key
		...
		invoke(
			...
			&[
				taker_temp_token_account.clone(),
		...
		let close_pda_temp_account_ix = spl_token::instruction::close_account(
			token_program.key,
			pda_temp_token_account.key,
			initializer.key,
			&pda,
